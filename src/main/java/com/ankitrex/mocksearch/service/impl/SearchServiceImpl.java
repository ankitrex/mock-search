...
public class SearchServiceImpl implements SearchService {
	...
	public List<SearchResult> performSearchOnIndex(String query, Integer maxResults) {
		...
		// get keyword token and get submap of keywordInvertedIndex that begins with
		// input keyword
		...
		for (Entry<String, List<Integer>> entry : keywordMatchMap.entrySet()) {
			...
			// kat matched with kate will have more score than kat matched with katelyn
		...
		if (scores.size() < maxResults) {
			...
			// generate query tokens for search string and get all the users that contain
			// the token
			...
			for (String token : queryTokens) {
				...
				if (ngramInvertedIndex.containsKey(token)) {
					...
					// length of the token matched will directly affect the score
		...
		// sort the scores map, limit the number of results, fetch data from user
		...
		return scores.entrySet().stream()
				...
				.limit(maxResults).map(x -> formatUserResult(users.get(x.getKey()), x.getValue().doubleValue()))
	 ...
	 * @return - SearchResult - formatted result
	 */
